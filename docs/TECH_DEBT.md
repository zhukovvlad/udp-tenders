...
- [ ] **MSW-хендлеры admin-эндпоинтов возвращают статические фикстуры**
  Хендлеры `/api/admin/*` в `frontend/src/test/handlers.ts` (дефолтные `GET organizations`,
  `GET users`, `PATCH`) отдают фиксированные данные и не отражают входящие `q`/`page`/`page_size`/body.
  Из-за этого happy-path тесты пройдут даже при неверной сериализации query/тела. Тесты, которым важна
  проверка контракта (поиск, пагинация, редактирование), уже переопределяют хендлер через `server.use`
  со спаем — это корректный паттерн. Дефолтные хендлеры стоит сделать «эхо»-образными для повышения
  ценности будущих тестов.
