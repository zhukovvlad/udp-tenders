...
Разрешение на запись в Neon из выбора `env` **не** следует — оно отдельное, иначе
защита тихо снималась бы тому, кто Neon вообще не трогает.
...
### Guard от записи в Neon

[`backend/db_guard.py`](../backend/db_guard.py) отсекает мутирующие операции,
нацеленные на Neon, **до** попытки коннекта. Мотив: `DATABASE_URL` в `.env`
указывает на прод, и команда, набранная по привычке мимо `just`, уехала бы туда.

Точки, где guard подключён:

| Точка | Что закрывает |
|---|---|
| `alembic/env.py` | любой DDL — модуль исполняется до ветвления online/offline, выше `engine_from_config` |
| `cli.py` | `create-superuser`, `create-org`, `create-user` — до открытия сессии |
| `main.py` (`_sweep_stuck_documents`) | startup-sweep: он на каждом старте переводит `pending`/`processing` в `error`, так что `uv run uvicorn main:app` вместо `just dev-backend` испортил бы живые прод-документы |
...
поэтому integration-тестам не нужно разрешение из-за прод-строки в `.env`.

**Следствие для деплоя:** прод работает на Neon, значит в прод-окружении
`APP_ENV=prod` обязателен — иначе приложение не стартует. Это осознанная
цена: явная переменная в конфиге прода против шанса испортить его с dev-машины.

Разрешение — `APP_ENV=prod` (роль и есть разрешение, цели не проверяются) либо
запись в `DB_EXTRA_TARGETS` для долгоживущих dev-целей. Их выставляют:

- прод-окружение — `APP_ENV=prod` (см. выше);
- `conftest.py` — разрешение приходит из конфига (`DB_EXTRA_TARGETS`), а не из
  рантайм-мутации: если `TEST_DATABASE_URL` указывает на цель, отличную от
  loopback, она должна быть в `DB_EXTRA_TARGETS`.

Обратите внимание: `db_target=env` разрешение **не** выдаёт. Осознанная миграция
прода с дев-машины выглядит так: `APP_ENV=prod just db_target=env db-migrate`.

Сравнение идёт по **суффиксу домена** в hostname, не подстрокой во всём DSN:
`myneon.techie.example.com` не Neon, пароль с `neon.tech` внутри не даёт ложного
срабатывания. В текст ошибки попадает только хост, без креденшелов — эти строки
уходят в логи и CI-вывод.

Guard не покрывает сырой SQL — например `DROP SCHEMA` в conftest. Там два
собственных рубежа: `TEST_DATABASE_URL` не должен совпадать с `DATABASE_URL`
(читается через `Settings()`, а не `os.getenv` — `pytest-dotenv` грузит только
`.env.test`, где `DATABASE_URL` нет, и проверка через `os.getenv` была бы
мёртвой), и имя целевой БД обязано оканчиваться на `_test`. Второй рубеж важен
именно теперь: `udp_dev` и `udp_test` различаются четырьмя символами.
