...
**Исключение:** ключи `host`, `hostaddr`, `port`, `dbname` и `service` в query-строке
делают цель нераспознанной безусловно (`UNKNOWN_HOST`-форма, не совпадает ни с loopback,
ни с записью `DB_EXTRA_TARGETS`) — это цель-определяющие для psycopg/libpq ключи, а не
опции соединения:

- `host`/`port`/`dbname` в query подставляются SQLAlchemy-диалектом поверх netloc при
  сборке connect-args, замещая, а не дополняя его;
- `hostaddr` — отдельный libpq-параметр: если заданы И `host`, И `hostaddr`, реальный
  сетевой адрес — `hostaddr`, `host` используется только для TLS-верификации имени;
- `service` тянет host/port/dbname из `pg_service.conf`, файла, которого guard не видит.

(см. спеку §14 — заменяет более ранний, точечный механизм §13). `#` и второй `/` в пути
(`.../dev#archive`, `...//dev`) — часть имени БД **буквально**, а не разделитель fragment
или пути: SQLAlchemy (и реальный psycopg-коннект) не знает понятия URL-fragment вовсе,
поэтому запись `DB_EXTRA_TARGETS` для `dev` не пропускает DSN, целящийся на
`dev#archive`/`/dev`. Пустой или неразбираемый хост даёт маркер `UNKNOWN_HOST`
