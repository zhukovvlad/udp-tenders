...
`ALLOW_NEON_WRITES=1` обязателен — иначе приложение не стартует. Это осознанная
цена: явная переменная в конфиге прода против шанса испортить его с dev-машины.

Разрешение — `ALLOW_NEON_WRITES=1`. Его выставляют:

- рецепт `db-test-migrate` (Neon test-ветка);
- `conftest.py` — точечно и только если `TEST_DATABASE_URL` действительно Neon,
  с откатом на teardown (`pytest.MonkeyPatch`), чтобы process-global переменная
  не протекала в тесты самого guard'а;
- прод-окружение (см. выше).

Обратите внимание: `db_target=env` разрешение **не** выдаёт. Осознанная миграция
в Neon выглядит так: `ALLOW_NEON_WRITES=1 just db_target=env db-migrate`.
