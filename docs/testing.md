...
**Предусловие перед барьером (a): нераспознанная цель — громкий отказ, не тихий skip.**
Если `TEST_DATABASE_URL` нельзя доверенно резолвить (окружение процесса несёт
`PGHOSTADDR`/`PGSERVICE`, `PGPORT` вне 0-65535, либо сам DSN не разбирается/несёт
цель-определяющий query-ключ), `_resolve_connect_target` для НЕГО и для `DATABASE_URL`
одинаково даёт `None` — проверка `PGHOSTADDR`/`PGSERVICE` в нём безусловна и не зависит от
конкретного DSN, поэтому отравленное окружение обесценивает резолв обеих сторон сравнения
одинаково. Без явной проверки барьер (a) увидел бы у обеих сторон одну и ту же
`UNKNOWN_HOST`-форму, решил бы «цели совпадают» и молча ушёл в `pytest.skip` — безопасно
для БД (`DROP SCHEMA` не выполнится), но съедает интеграционный слой незаметно и ломает
инвариант «ровно 6 пропущенных» так, что зелёный прогон выглядит нормальным. `db_engine`
(`conftest.py`) поэтому падает `RuntimeError` ДО барьера (a), если `TEST_DATABASE_URL`
нераспознан, — оба барьера (a) и (b) при этом не ослаблены, это предварительная проверка их
собственной предпосылки (доверенного резолва).
