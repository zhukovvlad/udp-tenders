...
### Task 2: MANUAL — `DB_EXTRA_TARGETS` в `backend/.env`

> **ВЫПОЛНЯЕТ ЧЕЛОВЕК. АГЕНТ НЕ ЖДЁТ МОЛЧА.**
>
> Агент обязан остановиться на этой задаче, вывести инструкцию ниже и **явно запросить подтверждение**, что шаг выполнен. Не «продолжу и надеюсь», не тихое ожидание, не пропуск с пометкой «сделаете позже». Task 3 без этого шага ломает рабочий путь.
