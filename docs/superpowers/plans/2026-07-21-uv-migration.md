...
- [ ] **Step 5: Проверить, что прямые версии в lock совпадают с текущими**

Run: `cd backend && uv pip list --format=freeze 2>/dev/null | grep -Ei "^(fastapi|pillow|python-multipart|pydantic-settings)=="`
(либо открыть `uv.lock` и найти `name = "fastapi"` → `version = "0.139.2"`)
Expected: `fastapi==0.139.2`, `pillow==12.3.0`, `python-multipart==0.0.31`, `pydantic-settings==2.14.2` (security-фиксы на месте).

- [ ] **Step 6: Заменить прямые зависимости на `>=` (проход 2)**
...
- [ ] **Step 7: Повторный lock (проход 2) — версии должны сохраниться**

Run: `cd backend && uv lock`
Expected: при существующем `uv.lock` uv сохраняет уже зафиксированные версии; в выводе нет апгрейдов прямых зависимостей (нет строк вида `Updated fastapi ...`). Exit 0. Повторно проверить `version = "0.139.2"` для fastapi в `uv.lock` — не изменилась.

- [ ] **Step 8: Синхронизировать окружение**
...
- [ ] **Step 4: Локальная валидация YAML**

Run: `cd backend && uv run python -c "import yaml,sys; yaml.safe_load(open('../.github/workflows/backend-tests.yml')); print('yaml ok')"`
Expected: `yaml ok`, exit 0. (Полная проверка CI — на PR, п. Task 6.)
...
backend/.venv/
```
(scoped — окружение создаётся рядом с backend `pyproject.toml`.)
...
- [ ] **Step 3: testing.md — версия pytest**

В `docs/testing.md` стр. 130: `- **pytest 8** + ...` → `- **pytest 9** + ...`.
...
Проверить, что workflow `backend-tests` на PR прошёл (setup-uv + uv sync --locked + uv run pytest). Если `--locked` падает на рассинхроне lock/pyproject — выполнить `cd backend && uv lock`, закоммитить обновлённый `uv.lock`, запушить.
...
**Placeholder scan:** версия setup-uv/uv в Task 3 намеренно подтверждается по доке (Step 1) — это осознанный шаг с ориентирами (v8.1.0 / 0.11.29), не placeholder. Числа passed-тестов даны с оговоркой «или текущие». Прочих TBD/TODO нет.
