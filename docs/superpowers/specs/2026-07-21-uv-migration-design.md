...
2. `uv lock` → `uv.lock` зафиксирует ровно текущее окружение.
...
Итог: те же версии, что и до миграции (включая security-фиксы прошлой задачи:
`python-multipart 0.0.31`, `pillow 12.3.0`, `python-dotenv 1.2.2`,
`pydantic-settings 2.14.2`), но теперь с `uv.lock` и `>=` границами.
...
На dev-машине и в CI должен быть установлен `uv` (standalone-инсталлятор / winget /
pip). Это единственный новый инструмент — запрошен пользователем явно.

## Порядок работ

1. Добавить `[project]`/`[dependency-groups]`/`[tool.uv]` в pyproject с версиями как
   `==` (шаг 1 двухпроходного lock).
2. `uv lock` (проход 1) → зафиксировать текущее окружение.
3. Заменить прямые зависимости на `>=` floors; `uv lock` (проход 2) → версии
   сохраняются.
4. Создать `backend/.python-version` (`3.12`).
