...
- calc_role: роль позиции в расчёте стоимости (только для item_type="material"):
  ...
  - "exclude" — сопутствующие позиции и услуги, которые не formируют стоимость материала и не распределяются пропорционально: цементное молоко, простой миксера, мойка миксера, проволока вязальная, разовые услуги и любые позиции, не являющиеся частью самого материала
            ...
            for item_idx, item in enumerate(inv_data.get("items", [])):
                ...
                if item.get("item_type") == "material" and item.get("material_class"):
                    mc = crud.get_or_create_material_class(
                        ...
                        calc_role=item.get("calc_role") or "base",
                    )
